...
/// @title RevertExtension
/// @notice Test extension that always reverts from `beforeSwap` (core revert-bubbling tests).
contract RevertExtension is IMetricOmmExtensions {
  error DeliberateExtensionRevert();
  error DeliberateAfterSwapExtensionRevert();
  ...
  function beforeSwap(address, address, bool, int128, uint128, uint256, uint128, uint128, bytes calldata)
    ...
    returns (bytes4)
  {
    revert DeliberateExtensionRevert();
  }
  ...
  function afterSwap(
    ...
    revert DeliberateAfterSwapExtensionRevert();
  }
}
