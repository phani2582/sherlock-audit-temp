...
`fallback()` expects (no deadline prefix — each word carries its own timestamp):

```text
[word0(32)][word1(32)]...
```

where every `wordN` is a slot word as described above. Calldata length must be a
non-zero multiple of 32 (`BadCalldataLength` otherwise).

## Signature Payload (Off-chain → On-chain)

`updateBySignature(feedCreator, newSlotValue, signature)` expects `newSlotValue` to be a single slot word (same layout), signed by `feedCreator` over:
