...
contract PriceProviderFactoryL2 is AccessControl, Multicall, IPriceProviderFactoryL2 {
    ...
    function addProvider(address provider) external override onlyRole(ADMIN_ROLE) {
        require(PriceProviderL2(provider).factory() == address(this));
    ...
    function setConfidence(
        ...
        for (uint256 i; i < l; ++i) {
            ...
            PriceProviderL2(providers[i]).setConfidenceParam(values[i]);
        }
    }
        ...
        returns (address[] memory providers, uint256[] memory updatableAfter, uint256 total)
    {
        ...
        for (uint256 i; i < count; ++i) {
            PriceProviderL2 p = PriceProviderL2(set.at(offset + i));
        ...
        returns (address[] memory providers, uint256[] memory updatableAfter, uint256 total)
    {
        ...
        for (uint256 i; i < count; ++i) {
            PriceProviderL2 p = PriceProviderL2(_providers.at(offset + i));
            providers[i] = address(p);
            updatableAfter[i] = p.lastConfidenceUpdate() + p.CONFIDENCE_COOLDOWN();
        }
    }
